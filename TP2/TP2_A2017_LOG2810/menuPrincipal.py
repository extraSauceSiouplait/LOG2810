...
for i in range(50):
    ...
    requests.traiter_les_requetes("requetes1.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes2.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes3.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes4.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes5.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes6.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes7.txt",automate,fleet,history)

    requests.traiter_les_requetes("requetes8.txt",automate,fleet,history)
    while(not requests.request_queue.empty()):
        requests.traiter_les_requetes("r0.txt", automate, fleet, history)

    stat += float(history.n_processed_requests)
    requests.clear_all()
    initialize_fleet_TP(fleet, history)

stat = stat/history.n_completed_cycles
print("nb moyen de cycles:" + str(stat))
