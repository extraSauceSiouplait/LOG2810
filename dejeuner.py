    ...
    if noeud in ingredient:                                         #Si la recette a des ingredients
        for x in ingredient[noeud]:                                 #Parcourir tout les ingredients
            if x in ingredient and firstTime:                       #Si l'ingredient a lui meme des ingredients
                if x in ingredient[x]:                              #Pour eviter boucle infini, ne repeter qu'une fois si la recette est aussi dans sa propre liste d'ingredient
                    firstTime = 0
...
while str != "c":
    lettre = raw_input("(a) Creer et afficher le graphe des recettes.\n"
